...
export async function pickImage(upload = false, roundId = null) {
    try {
        // Request permission
    ...
    try {
        ...
        if (path === "profilePicture") {
            ...
            if (roundDoc.exists()) {
                ...
                await updateDoc(roundDocRef, { images: [...(roundData.images || []), url] });
            }
        }
...
export async function getUser() {
    try {
        ...
        return userDoc.exists() ? { uid: auth.currentUser.uid, email: auth.currentUser.email, ...userDoc.data() } : null;
    } catch (error) {
...
export async function getRounds(q = 'date-desc') {
    ...
    try {
        const snapshot = await getDocs(query(collection(db, "users", auth.currentUser.uid, "rounds"), orderBy(field, direction)));
...
export async function getAsyncUser() {
    try {

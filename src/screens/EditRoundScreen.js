...
export default function EditRoundScreen({ route }) {
    ...
    return (
        <KeyboardAwareScrollView style={{backgroundColor: theme.colors.surface}} contentContainerStyle={{justifyContent: 'center', alignItems: 'center', alignContent: 'space-around', flexGrow: 1, backgroundColor: theme.colors.surface}}>
            <Portal>
                <Modal visible={loading} dismissable={false} dismissableBackButton={false} contentContainerStyle={{backgroundColor: theme.colors.surfaceVariant, paddingHorizontal: 30, paddingBottom: 45, paddingTop: 38, borderRadius: 10, margin: 20, width: '85%', alignSelf: 'center', gap: 75, justifyContent: 'center', alignItems: 'center'}}>
            ...
            <View style={{ width: '100%', alignContent: 'center', alignItems: 'center', justifyContent: 'center', margin: 15}}>
                <View style={{display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 15}} >
                    <Text variant="headlineSmall">Edit Round</Text>
                </View>
                <Input onChange={setCourse} type='search' value={course} inputRef={courseRef} nextRef={dateRef}>Course</Input>
                <View className={"max-w-96 self-center grow shrink w-full p-2 basis-full"}>
                    <View style={{borderRadius: 15, borderColor: theme.colors.onSurfaceVariant, backgroundColor: theme.colors.elevation.level0, width: '100%', padding: 10, display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
                ...
                <Input onChange={setScore} type='number' value={score} inputRef={scoreRef} nextRef={tempRef}>Score</Input>
                <Input onChange={setTemp} type='number' value={temp} inputRef={tempRef} nextRef={rainRef}>Temperature</Input>
                <Input onChange={setRain} type='number' value={rain} inputRef={rainRef} nextRef={windRef}>Rain</Input>
                <Input onChange={setWind} type='number' value={wind} inputRef={windRef} nextRef={notesRef}>Wind</Input>
                <Input onChange={setNotes} value={notes} inputRef={notesRef} nextRef={teesRef}>Notes</Input>
                <Input onChange={setTees} value={tees} inputRef={teesRef}>Tees</Input>
                <View style={{display: 'flex', justifyContent: 'center', alignItems: 'center', width: '80%', height: 'auto', marginVertical: 10}}>
                    <Carousel
                        width={320}
                        height={200}
                ...
                <Button mode="contained" style={{marginTop: 15}} onPress={updateDBRound}>Update Round</Button>
            </View>
